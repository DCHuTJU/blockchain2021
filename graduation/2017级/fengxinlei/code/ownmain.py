...
def checkdataset(info):
    ...
    fig, ax = plt.subplots()
    plt.ylim(0, 40)
    plt.plot(timestamp2, road_count2, c='b', label='$y = Vehicle Group Count$')
    plt.plot(timestamp2, everange2, c='g', label='$y = Average Scale$')
    plt.title(" Vehicle Group Count and Average Scale")
    plt.xlabel("Timestamp")
    plt.ylabel("Average Team Scale")
    plt.legend()
    plt.show()
        ...
        while count > 0:
           ...
           # print("Optimit01bag result:")
            ...
            if aimTask == 270:
                ...
                # print("***************************************")
                basecredit = 0.5
                overPaymentRatioList = optimit01bag.bagPaymentChangeCredit(allVehicleCount,bagResult, aimTask, teamsNum, testTask, taskCount, Cost, teamId, teamCredit,basecredit)
                print("overPaymentRatioList    ", overPaymentRatioList)
                ...
                # bagOverPayment.append((bagPayment-bagCost)/bagCost)

                Time = 4000
                count = -1
                break
                #heappush(resultQ, resultSort.CompareAble(aimTask, bagNum, bagCost, bagPayment))
            count -=1
        Time += 50

 #resultSort.showFigureCredit(aimTaskArray,bagOverPayment)
    resultSort.showOwnPaymentCostCredit(overPaymentRatioList)
